...
export const addCard = (name, url, bucketId) => async (dispatch) => {
  ...
  await fetch('/api/server.js/cards', {
    method: 'POST',
    headers: {
...
export const deleteCard = (cardId) => async (dispatch) => {
  await fetch(`/api/server.js/cards/${cardId}`, {
    method: 'DELETE',
  });
...
export const updateCard = (card) => async (dispatch) => {
  await fetch(`/api/server.js/cards/${card.id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const deleteMultipleCards = (cardIds) => async (dispatch) => {
  await Promise.all(
    cardIds.map((cardId) =>
      fetch(`/api/server.js/cards/${cardId}`, {
        method: 'DELETE',
      })
    )
  );
